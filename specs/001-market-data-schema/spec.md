...
**Feature Branch**: `001-market-data-schema`
**Created**: 2026-02-02
**Status**: Draft
...
- **FR-011**: 各フィールドの必須/任意は本仕様のスコープ外とし、別表（フィールド要件表）または金融商品カテゴリごとのサブ仕様で定義するものとする
...
- Q: ADRで多種多様なフィールドが定義されているが、個々のフィールドの必須/任意をこの仕様で定義すべきか？ → A: フィールドの必須/任意は本仕様のスコープ外とする。本仕様ではフィールド名の標準化（ADR準拠）とスキーマ構造の定義に集中し、必須/任意の定義は別表（フィールド要件表）または金融商品カテゴリごとのサブ仕様で管理する
